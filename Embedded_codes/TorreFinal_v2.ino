...
float ph_act;
float calibration_value = 22.5; //13.213
...
// Configurações MQTT
const char* mqttServer = "IP_adress";
const int mqttPort = "port";  // Porta padrão MQTT (lembrar que é do tipo inteiro)
...
void send_data_task(void *pvParameters) {
  while (1) {
    ...
    if (xSemaphoreTake(xSemaphore, portMAX_DELAY) == pdTRUE){
      ...
      char payload[256];
      sprintf(payload, "{\"Temperatura\": %.2f, \"CondutividadeEletrica\": %.0f, \"NivelBoiaAlta\": %d, \"NivelBoiaBaixa\": %d, \"pH\": %.2f}", tempC, conductivity, intermediary_buoy, low_buoy, pH);
